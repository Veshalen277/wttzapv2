...
try {
 ...
 $s->observe($source,'reaction',$user,'reaction',$date,0,false,1,$policy);
 $s->observe($source,'reaction',$user,'reaction',$date,0,true,2,$policy);
 $reaction=$s->rows('SELECT * FROM portal_points_events WHERE source=? AND source_ref=?','ss',[$source,'reaction'])[0];
 check((int)$reaction['awarded']===1,'Resolved reaction eligibility not credited');
 $s->observe($source,'reaction',$user,'reaction',$date,0,false,3,$policy);
 $net=$s->rows('SELECT SUM(delta) n FROM portal_points_ledger WHERE event_id=?','i',[$reaction['id']])[0];
 check((int)$net['n']===0,'Ineligible reaction not reversed');
