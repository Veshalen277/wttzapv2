...
- **History:** latest 100 team-update rows (setup entries are separate), with actor, reason, before, delta and after. Correct mistakes with a new movement; do not edit ledger rows.
...
## Presentation for different experience levels

The main screen shows live stock counts and team activity. Opening/setup balances are retained in a collapsed record-details section, not the recent-activity feed. Summary counts cover all active items, independently of the search filter. Items at zero appear only in Out of stock; Running low counts positive balances at or below the reorder threshold. No new migration is needed for this display update. See USER-GUIDE.md for everyday instructions.
