...
<header class="portal-header">
    <div class="portal-topbar">
        ...
        <div class="portal-account">
            ...
            $portalUnread=0;
            try {
                $notificationDb=\Portal\Connection::get();
                $notificationUser=\Portal\User::fromSession($_SESSION)->id;
                $notificationQuery=$notificationDb->prepare('SELECT COUNT(*) n FROM portal_notifications WHERE user_id=? AND read_at IS NULL');
                $notificationQuery->bind_param('i',$notificationUser);$notificationQuery->execute();
                $portalUnread=(int)$notificationQuery->get_result()->fetch_assoc()['n'];$notificationQuery->close();
            } catch(\Throwable $ignored) { /* Optional module: shell must keep working before installation. */ }
            ?>
            <a class="portal-icon-link portal-notification-link" href="/dashboard/notifications/index.php" aria-label="Notifications<?= $portalUnread?', '.$portalUnread.' unread':'' ?>" title="Notifications"><i class="bi bi-bell" aria-hidden="true"></i><?php if($portalUnread):?><span class="portal-notification-count"><?= $portalUnread>99?'99+':$portalUnread ?></span><?php endif; ?></a>
            <label class="portal-appearance"><span class="visually-hidden">Appearance</span>
                <select id="theme-select" aria-label="Appearance">
                    <option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option><option value="morning">Morning</option><option value="afternoon">Afternoon</option>
                </select>
            </label>
