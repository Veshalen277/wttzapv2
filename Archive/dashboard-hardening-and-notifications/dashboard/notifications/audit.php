...
try{
 ...
 $pending=(int)$db->query("SELECT COUNT(*) n FROM portal_points_ledger l WHERE l.id>$baseline AND l.delta<>0 AND NOT EXISTS(SELECT 1 FROM portal_notifications n WHERE n.user_id=l.user_id AND n.source='points' AND n.source_ref=CAST(l.id AS CHAR))")->fetch_assoc()['n'];
 $bad=(int)$db->query("SELECT COUNT(*) n FROM portal_notifications n LEFT JOIN portal_points_ledger l ON n.source_ref=CAST(l.id AS CHAR) WHERE n.source='points' AND (l.id IS NULL OR n.user_id<>l.user_id OR l.delta=0)")->fetch_assoc()['n'];
