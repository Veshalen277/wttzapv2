...
$issues=0;
foreach($store->rows('SELECT l.id FROM portal_points_ledger l LEFT JOIN portal_points_events e ON e.id=l.event_id WHERE e.id IS NULL') as $row){echo 'MISSING EVENT ledger '.$row['id']."\n";$issues++;}
foreach($store->rows('SELECT e.id FROM portal_points_events e LEFT JOIN users_tbl u ON u.id=e.user_id WHERE u.id IS NULL') as $row){echo 'MISSING EMPLOYEE event '.$row['id']."\n";$issues++;}
foreach($store->rows('SELECT e.id FROM portal_points_events e LEFT JOIN portal_points_ledger l ON l.event_id=e.id GROUP BY e.id,e.revision HAVING COUNT(l.id)<>e.revision OR MAX(l.revision)<>e.revision') as $row){echo 'REVISION MISMATCH event '.$row['id']."\n";$issues++;}
