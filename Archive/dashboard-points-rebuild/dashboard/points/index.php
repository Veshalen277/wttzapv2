...
$total=(int)$store->rows('SELECT COALESCE(SUM(delta),0) n FROM portal_points_ledger WHERE user_id=? AND occurred_at>=? AND occurred_at<?','iss',[$target,$from,$until])[0]['n'];
...
<section class="card points-summary"><span><?= $range==='all'?'All recorded activity':'This month' ?></span><strong><?= number_format($total) ?></strong><span>points</span><nav><a href="?user=<?= $target ?>&amp;range=month">This month</a> · <a href="?user=<?= $target ?>&amp;range=all">All time</a></nav></section>
<h2>Points by activity</h2><div class="points-grid"><?php foreach($breakdown as $row): ?><div class="card points-category"><span><?= Html::escape($pointPolicy['rules'][$row['kind']]['label']??$row['kind']) ?></span><strong><?= (int)$row['points'] ?></strong></div><?php endforeach; ?><?php if(!$breakdown): ?><p>No scored activity in this period.</p><?php endif; ?></div>
