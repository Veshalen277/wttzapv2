...
function handleDelete($con) {
    if (isset($_POST['delete_btn'])) {
        ...
        if ($stmt->execute()) {
            echo '<div class="alert alert-success mt-3" role="alert">Report deleted successfully</div>';
        } else {
            echo '<div class="alert alert-danger mt-3" role="alert">Error deleting report: ' . htmlspecialchars($con->error) . '</div>';
        }
...
function fetchReports($con) {
    $sql = "SELECT r.*, u.fullname, u.user_scale
            ...
            WHERE r.status != 'archived'"; // Fetch only non-archived reports
